    ...
    while True:
        if res.status_code != 200:
            if res.status_code == 404:
                logger.info('There is a problem with the request URL. Make sure that it is correct')
            else:
                logger.info("There was a problem retrieving data: " + res.text)
            trial += 1
            if trial >= RETRIES:
                raise IOError('Error while querying first_open report from AppsFlyer', res.text)
            logger.info("retry in 1 minute..")
            time.sleep(60)
        else:
            return res.text
